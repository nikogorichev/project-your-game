...
## Настя: Логин и регистрация (бек и фронт); отрисовать компоненты логи, реги, навбар(при создании сессии исчезает логин и регистрация, появляется логаут и личный кабинет), главная страница(без регистрации (/)), при создании сессии должно перекидывать на ручку /game


## Антон: отрисовка на ручке /game всех карточек из БД, генерация вопросов, создание компонента личного кабинета

## Коля: Модальные окна, проверка на правильность ответа, изменение счета у юзера, неактивность кнопки после ее 
...
('/game') - главная после регистрации (на ней отрисовка карточек)

('/profile') - profile

('/game/:id') - модальные окна
